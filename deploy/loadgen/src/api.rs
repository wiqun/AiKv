...
use crate::config::{ConfigPatch, Preset, WorkloadConfig};
use crate::engine::RuntimeState;
use crate::ui::{read_asset, StaticAsset};
...
fn static_response(dir: &std::path::Path, asset: StaticAsset) -> impl IntoResponse {
    match read_asset(dir, asset) {
        Ok(body) => (
            [
...
async fn index(State(app): State<AppState>) -> impl IntoResponse {
    static_response(&app.web_dir, StaticAsset::Index)
}

async fn app_css(State(app): State<AppState>) -> impl IntoResponse {
    static_response(&app.web_dir, StaticAsset::Css)
}

async fn app_js(State(app): State<AppState>) -> impl IntoResponse {
    static_response(&app.web_dir, StaticAsset::Js)
}

async fn wiqun_icon(State(app): State<AppState>) -> impl IntoResponse {
    static_response(&app.web_dir, StaticAsset::Svg)
}
...
mod tests {
    ...
    fn app() -> Router {
        let ui = crate::config::code_defaults();
        router(AppState {
            cfg: Arc::new(ArcSwap::from_pointee(ui.config)),
            state: Arc::new(RuntimeState::default()),
            presets: Arc::new(ui.presets),
            web_dir: crate_web_dir(),
        })
    }

    fn app_with(cfg: WorkloadConfig, state: Arc<RuntimeState>) -> Router {
        router(AppState {
    ...
    async fn index_serves_html() {
        ...
        assert!(
            html.contains("id=\"mode_badge\""),
            "顶栏应显示 Cluster/Single"
        );
        assert!(
            html.contains("id=\"task_info_btn\""),
            "运行中/暂停徽章内应有当前任务叹号"
        );
        assert!(html.contains("id=\"btn_primary\""), "顶栏只留一颗主按钮");
        assert!(
            html.contains("id=\"task_stop_btn\""),
            "终止任务放在状态徽章上"
        );
        ...
        assert!(
            html.contains("id=\"dirty_hint\""),
            "运行中改表单应有草稿未生效提示"
        );
        assert!(!html.contains("const PRESETS"), "预设不得写死在 HTML");
        assert!(
    ...
    async fn body_text(response: axum::response::Response) -> (StatusCode, String, String, String) {
    ...
    async fn serves_app_js() {
        let response = app()
            ...
            .await
            .unwrap();
        let (status, ctype, cache, js) = body_text(response).await;
        assert_eq!(status, StatusCode::OK);
        assert!(ctype.starts_with("text/javascript"), "{ctype}");
        assert_eq!(cache, "no-store");
        assert!(js.contains("renderPresetCards"), "侧栏必须按 API 渲染");
        assert!(js.contains("paused: true"), "运行中点暂停而不是拆泵");
        assert!(js.contains("paused: false"), "暂停后点继续恢复派发");
        assert!(
            js.contains("readForm(), running: true"),
            "启动必须提交整张表单, 不能只发 running"
        );
        assert!(
            js.contains("/api/commands"),
            "前端必须从引擎目录拉取可发压命令"
        );
        assert!(!js.contains("triggerReload"), "刷新按钮已删除, 轮询已足够");
    }

    #[tokio::test]
    async fn serves_app_css() {
        let response = app()
            .oneshot(Request::get("/app.css").body(Body::empty()).unwrap())
            .await
            .unwrap();
        let (status, ctype, cache, css) = body_text(response).await;
        assert_eq!(status, StatusCode::OK);
        assert!(ctype.starts_with("text/css"), "{ctype}");
        assert_eq!(cache, "no-store");
        assert!(css.contains("--accent"));
    }

    #[tokio::test]
    async fn static_missing_file_is_404() {
        let dir =
            std::env::temp_dir().join(format!("loadgen-web-empty-api-{}", std::process::id()));
    ...
    #[tokio::test]
    async fn get_config_includes_presets() {
        let json = body_json(
            app()
                .oneshot(Request::get("/api/config").body(Body::empty()).unwrap())
                .await
        ...
        assert_eq!(json["presets"][0]["id"], "balanced");
        assert_eq!(json["presets"][0]["config"]["target_ops"], 8000);
        assert!(
            json["presets"][0]["config"].get("running").is_none()
                || json["presets"][0]["config"]["running"] == false
        );
    }
    ...
    async fn lists_dispatchable_aikv_commands() {
        ...
        let names: Vec<&str> = json["commands"]
            ...
            .collect();
        for need in [
            "GET", "HGET", "HSET", "MSET", "JSON.GET", "JSON.SET", "LPUSH", "ZADD",
        ] {
            assert!(names.contains(&need), "missing {need}");
        }
        assert!(!names.contains(&"FLUSHALL"));
        assert!(!names.contains(&"KEYS"), "KEYS 不进入 mix 目录");
    ...
    async fn put_without_start_does_not_bump_epoch() {
        ...
        let app = app_with(WorkloadConfig::default(), state.clone());
        let request = Request::put("/api/config")
            .header("content-type", "application/json")
    ...
    async fn put_pause_keeps_running_and_does_not_bump_epoch() {
        ...
        let app = app_with(cfg, state.clone());
        let request = Request::put("/api/config")
            .header("content-type", "application/json")
    ...
    async fn put_start_failure_does_not_bump_epoch() {
        ...
        let request = Request::put("/api/config")
            .header("content-type", "application/json")
            .body(Body::from(
                r#"{"running":true,"mode":"single","endpoint":"127.0.0.1:1","timeout_ms":50}"#,
            ))
            .unwrap();
        let response = app.oneshot(request).await.unwrap();
        assert_eq!(response.status(), StatusCode::BAD_REQUEST);
