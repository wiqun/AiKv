...
| 混合 | set/get/del/mget/incr/expire 及 HGET/MSET/JSON.GET 等 | 40/40/5/10/3/2 | 权重自动归一化; 控制台从 `/api/commands` 拉取引擎可发压的 aikv 客户端命令. `KEYS` 不在目录中; 选 INFO / SMEMBERS 等重命令会弹窗提醒, 仍可加入 mix |
