...
pub async fn generate(
    ...
    match deploy_type {
        "single" => generate_single_deployment(project_dir, output_dir)?,
        "cluster" => generate_cluster_deployment(project_dir, output_dir)?,
        _ => {
            return Err(anyhow!(
...
fn generate_single_deployment(_project_dir: &Path, output_dir: &Path) -> Result<()> {
    ...
    let config_content = generate_single_config();
...
fn generate_cluster_deployment(_project_dir: &Path, output_dir: &Path) -> Result<()> {
    ...
    for i in 1..=6 {
        let config_content = generate_cluster_node_config(i);
        fs::write(
...
services:
  aikv1:
    ...
    hostname: aikv1
    command: ["--host", "0.0.0.0", "--port", "6379"]
    ports:
      - "6379:6379"
      - "16379:16379"
    volumes:
  ...
  aikv2:
    ...
    hostname: aikv2
    command: ["--host", "0.0.0.0", "--port", "6380"]
    ports:
      - "6380:6380"
      - "16380:16380"
    volumes:
  ...
  aikv3:
    ...
    hostname: aikv3
    command: ["--host", "0.0.0.0", "--port", "6381"]
    ports:
      - "6381:6381"
      - "16381:16381"
    volumes:
  ...
  aikv4:
    ...
    hostname: aikv4
    command: ["--host", "0.0.0.0", "--port", "6382"]
    ports:
      - "6382:6382"
      - "16382:16382"
    volumes:
  ...
  aikv5:
    ...
    hostname: aikv5
    command: ["--host", "0.0.0.0", "--port", "6383"]
    ports:
      - "6383:6383"
      - "16383:16383"
    volumes:
  ...
  aikv6:
    ...
    hostname: aikv6
    command: ["--host", "0.0.0.0", "--port", "6384"]
    ports:
      - "6384:6384"
      - "16384:16384"
    volumes:
